#!/usr/bin/env python3
"""
LLM Medical Term Correction Script

This script uses LLM models to correct medical terms in ASR transcripts.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('llm_medical_correction.log'),
...
class OpenAICompatibleClient(LLMClient):
    ...
    def generate(self, prompt: str, max_retries: int = 3) -> Optional[str]:
        ...
        data = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": "You are a medical transcription specialist."},
...
def correct_medical_terms(transcript: str, client: LLMClient, prompt_template: str) -> Optional[str]:
    """Correct medical terms in transcript using LLM"""
    try:
...
def process_single_file(args: tuple) -> Dict[str, Any]:
    ...
    try:
        ...
        corrected_transcript = correct_medical_terms(original_transcript, client, prompt_template)
...
def main():
    parser = argparse.ArgumentParser(description="LLM Medical Term Correction")
    ...
    parser.add_argument("--prompt", default="You are a medical transcription specialist. Please correct any medical terms, drug names, anatomical terms, and medical procedures in the following ASR transcript. Maintain the original meaning and context. Only correct obvious medical errors and standardize medical terminology. Return only the corrected transcript without explanations.",
                       help="Custom prompt for medical correction")
    ...
    process_args = [
        (file_path, output_dir, client, args.prompt)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.batch_size) as executor:
        ...
        with tqdm(total=len(transcript_files), desc="Processing transcripts") as pbar:
    ...
    summary_file = output_dir / "medical_correction_summary.json"
