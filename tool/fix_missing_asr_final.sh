...
# --- Fix Missing ASR Files Script (Final) ---
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --models)
            ...
            ;;
        -h|--help)
            echo "Fix Missing ASR Files Script (Final)"
            echo ""
...
cat > "$FIX_OUTPUT_DIR/analyze_missing_final.py" << 'EOF'
#!/usr/bin/env python3
...
def parse_model_analysis(analysis_file):
    ...
    for i, line in enumerate(lines):
        ...
        if line == 'Successful files:' and current_model:
        ...
        if in_successful_section and line.startswith('  ✓ '):
            ...
            if current_model:
                ...
                # Format: model_filename.txt -> filename
                if '_' in filename:
                    # Split by first underscore to separate model name from filename
                    ...
                    # Handle case where filename doesn't have underscore
                    base_filename = filename.replace('.txt', '')
                    model_files[current_model].add(base_filename)
        
        # End successful section when we encounter a new section or empty line followed by non-successful line
        if in_successful_section and line == '':
            # Check if next line is not a successful file
            if i + 1 < len(lines):
...
$PYTHON_EXEC "$FIX_OUTPUT_DIR/analyze_missing_final.py" \
