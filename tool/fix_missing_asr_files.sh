...
# --- Fix Missing ASR Files Script ---
# This script analyzes model_file_analysis.txt to identify missing files
# and re-runs ASR models for those files with detailed error analysis.
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --models)
            ...
            ;;
        -h|--help)
            echo "Fix Missing ASR Files Script"
            echo ""
            ...
            echo ""
            echo "Examples:"
            echo "  $0 --pipeline_output_dir /path/to/pipeline_results --fix_output_dir /path/to/fix_results"
            echo "  $0 --pipeline_output_dir /path/to/pipeline_results --fix_output_dir /path/to/fix_results --models large-v3,canary-1b"
            exit 0
            ;;
        *)
...
echo "Fix output directory: $FIX_OUTPUT_DIR"
echo "Models to fix: ${MODEL_ARRAY[*]}"
echo "==============================================="
echo ""
...
# Check if model_file_analysis.txt exists
...
def parse_model_analysis(analysis_file):
    """Parse model_file_analysis.txt to extract missing files"""
    ...
    with open(analysis_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Check for successful files section
            if line == 'Successful files:':
                in_successful_section = True
                continue
            
            # Parse successful files
            if in_successful_section and line.startswith('  ✓ '):
                filename = line.replace('  ✓ ', '').strip()
                if current_model:
                    if current_model not in missing_files:
                        missing_files[current_model] = []
                    # We'll populate this with expected files later
            ...
            elif 'Successful files:' in line and 'files:' in line:
                parts = line.split(':')
                if len(parts) == 2 and current_model in model_stats:
                    model_stats[current_model]['successful'] = int(parts[1].strip())
    
    return missing_files, model_stats

def get_expected_files(transcript_dir, ground_truth_file):
    """Get expected files from ground truth"""
    import pandas as pd
...
def find_missing_files(transcript_dir, expected_files, models):
    """Find missing files for each model"""
    missing_files = defaultdict(list)
    
    for model in models:
        for expected_file in expected_files:
            transcript_file = os.path.join(transcript_dir, f"{model}_{expected_file}.txt")
...
def analyze_audio_files(audio_dir, missing_files):
    """Analyze audio files to understand why they might be missing"""
    analysis = {}
    ...
    for model, files in missing_files.items():
        ...
        for file in files:
            ...
            if not os.path.exists(audio_file):
                ...
                # Check if it's a long audio file
                try:
                    import librosa
...
def main():
    ...
    parser.add_argument("--transcript_dir", required=True, help="Transcript directory")
    ...
    # Get expected files
    expected_files = get_expected_files(args.transcript_dir, args.ground_truth_file)
    
    # Find missing files
    missing_files = find_missing_files(args.transcript_dir, expected_files, models)
    ...
    for model in models:
        ...
        print(f"{model}: {missing_count} missing files")
        if missing_count > 0:
            for file in missing_files[model][:5]:  # Show first 5
                print(f"  - {file}")
...
$PYTHON_EXEC "$FIX_OUTPUT_DIR/analyze_missing.py" \
    ...
    --transcript_dir "$PIPELINE_OUTPUT_DIR/merged_transcripts" \
...
# Create Python script to generate re-run commands
...
# Create Python script to generate detailed report
