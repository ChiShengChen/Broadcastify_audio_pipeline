...
# --- Fix Missing ASR Files Script (Corrected) ---
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --models)
            ...
            ;;
        -h|--help)
            echo "Fix Missing ASR Files Script (Corrected)"
            echo ""
...
cat > "$FIX_OUTPUT_DIR/analyze_missing_correct.py" << 'EOF'
#!/usr/bin/env python3
...
def parse_model_analysis(analysis_file):
    ...
    with open(analysis_file, 'r', encoding='utf-8') as f:
        for line in f:
            ...
            # Reset section flag when we encounter a new section
            if line.startswith('  ') and not line.startswith('  ✓ ') and in_successful_section:
                in_successful_section = False
...
$PYTHON_EXEC "$FIX_OUTPUT_DIR/analyze_missing_correct.py" \
