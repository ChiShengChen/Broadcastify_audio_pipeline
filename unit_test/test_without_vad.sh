...
TEST_OUTPUT_DIR="/media/meow/One Touch/ems_call/test_no_vad_$(date +%Y%m%d_%H%M%S)"
TEST_GROUND_TRUTH="/media/meow/One Touch/ems_call/test_ground_truth_no_vad.csv"
...
# 運行基本ASR測試（不使用VAD和長音頻分割）
echo "🔧 運行基本ASR測試（不使用VAD和長音頻分割）"
echo "----------------------------------------"
echo "運行命令:"
