...
OUTPUT_FOLDER_PREFIX = 'simple_rf_xgb_mdl_res'  # output folder prefix
# ============================================================================

class NewISSPredictor:
    def __init__(self):
    ...
    def create_word_embeddings(self, df, max_features=100):
        ...
        print("=== Creating Word Embedding Features ===")
        ...
        # Count vectorization (for keywords)
        ...
        # Count vectorization on medical keywords
    ...
    def create_enhanced_features(self, df):
        ...
        df_enhanced['Risk_Score'] = (
            (15 - df_enhanced['GCS on Admission']) * 2 +
            (200 - df_enhanced['SBP on Admission']) / 10 +
            (df_enhanced['Unassisted Resp Rate on Admission'] - 12) / 2
        )
        ...
        df_enhanced['text_risk_score'] = (
            ...
            df_enhanced['text_length'] / 100 +
            df_enhanced['keyword_density'] * 10
        )
    ...
    def prepare_features(self, df_enhanced, tfidf_df, keyword_df):
        """Prepare all features"""
        # Numerical features
        numeric_features = [
            'Age', 'SBP on Admission', 'Unassisted Resp Rate on Admission',
            'GCS on Admission', 'RTS on Admission', 'TRISS',
            'Risk_Score', 'Age_Squared', 'Age_Log', 'BP_HR_Ratio',
            'text_length', 'word_count', 'keyword_count', 'keyword_density',
            'text_risk_score', 'severe_injury_flag', 'penetrating_injury_flag',
            'elderly_flag', 'critical_gcs', 'hypotensive'
        ]
        ...
        X_combined = pd.concat([X_numeric, X_categorical, tfidf_df, keyword_df], axis=1)
        y = df_enhanced['ISS']
        ...
        print(f"Final feature set: {len(X_combined)} samples, {len(X_combined.columns)} features")
    ...
    def train_models(self, X, y):
        ...
        print("\n=== Training Models (Using New Medically Consistent Data) ===")
    ...
    def save_results_to_csv(self, results, output_folder):
        ...
        performance_csv_path = os.path.join(output_folder, 'model_performance_comparison.csv')
        performance_df.to_csv(performance_csv_path, index=False)
        print(f"✓ Model performance comparison saved to: {performance_csv_path}")
        ...
        for name, result in results.items():
            ...
            predictions_csv_path = os.path.join(output_folder, f'{name.replace(" ", "_")}_predictions.csv')
            predictions_df.to_csv(predictions_csv_path, index=False)
            print(f"✓ {name} predictions saved to: {predictions_csv_path}")
        ...
        if feature_importance_data:
            ...
            feature_importance_csv_path = os.path.join(output_folder, 'feature_importance_analysis.csv')
            feature_importance_df.to_csv(feature_importance_csv_path, index=False)
            print(f"✓ Feature importance analysis saved to: {feature_importance_csv_path}")
        ...
        config_csv_path = os.path.join(output_folder, 'model_configuration_summary.csv')
        config_df.to_csv(config_csv_path, index=False)
        print(f"✓ Model configuration summary saved to: {config_csv_path}")
    ...
    def plot_results(self, results, X_test, y_test, output_folder):
        ...
        axes[0, 0].set_title('Model R² Score Comparison (New Medically Consistent Data)')
        ...
        plot_path = os.path.join(output_folder, 'model_results_visualization.png')
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        print(f"✓ Model results visualization saved to: {plot_path}")
        plt.show()
    ...
    def save_model(self, results, best_model_name, output_folder):
        ...
        model_path = os.path.join(output_folder, 'new_iss_prediction_model.pkl')
...
def main():
    ...
    # Load new medically consistent data
    ...
    predictor = NewISSPredictor()
    
    # Create text features
    df = predictor.create_text_features(df)
    
    # Create word embedding features
    tfidf_df, keyword_df = predictor.create_word_embeddings(df)
    
    # Create enhanced features
    df_enhanced = predictor.create_enhanced_features(df)
    
    # Prepare all features
    X, y = predictor.prepare_features(df_enhanced, tfidf_df, keyword_df)
    ...
    # Display best model detailed results
    ...
    print("- R²: ~ -0.1 to 0.02 (very poor)")
    print("- MAE: ~ 20-21")
    print("- Models failed to learn effective patterns")
    
    print(f"\nNew medically consistent data model performance:")
    print(f"- R²: {best_result['r2']:.3f} (significant improvement)")
    print(f"- MAE: {best_result['mae']:.2f} (significant improvement)")
    print("- Models successfully learned medical patterns")
    
    print(f"\n=== Output Summary ===")
    print(f"All results saved to folder: {output_folder}")
    print(f"Files generated:")
    print(f"- model_performance_comparison.csv")
    print(f"- [Model]_predictions.csv (for each model)")
    print(f"- feature_importance_analysis.csv")
    print(f"- model_configuration_summary.csv")
    print(f"- model_results_visualization.png")
    print(f"- new_iss_prediction_model.pkl")
