...
class GPTOSS120BModel:
    ...
    def load_model(self):
        """加载模型"""
        try:
            logger.info(f"Loading gpt-oss-120b model: {self.model_name}")
            ...
            logger.info("gpt-oss-120b model loaded successfully!")
...
def main():
    """主函数"""
    if len(sys.argv) < 4:
        print("Usage: python llm_gpt_oss_120b.py <input_dir> <output_dir> <prompt> [temperature] [max_new_tokens]")
        sys.exit(1)
    ...
    try:
        # 创建模型
        logger.info("Initializing gpt-oss-120b model...")
        model = GPTOSS120BModel(device="cuda", temperature=temperature, max_new_tokens=max_new_tokens)
        ...
        with tqdm(total=len(transcript_files), desc="Processing with gpt-oss-120b") as pbar:
        ...
        summary_path = Path(output_dir) / "gpt_oss_120b_summary.json"
