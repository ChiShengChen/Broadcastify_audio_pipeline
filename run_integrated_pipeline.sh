...
# --- Integrated ASR Pipeline with Audio Preprocessing ---
# This script integrates audio preprocessing with the enhanced ASR evaluation process:
# 1. AUDIO PREPROCESSING: Ensure compatibility with all ASR models
# 2. OPTIONAL VAD: Extract speech segments from audio files
# 3. ASR: Transcribe audio (original files or VAD segments)  
# 4. EVALUATION: Compare transcripts against ground truth
...
OUTPUT_DIR="/media/meow/One Touch/ems_call/integrated_pipeline_results_${TIMESTAMP}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --enhanced-preprocessor-mode)
            ...
            ;;
        -h|--help)
            echo "Integrated ASR Pipeline with Audio Preprocessing"
            echo ""
...
if [ "$USE_AUDIO_PREPROCESSING" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "  Output directory: $PREPROCESSED_AUDIO_DIR" >> "$ERROR_LOG_FILE"
...
if [ "$USE_LONG_AUDIO_SPLIT" = true ]; then
    ...
    $PYTHON_EXEC long_audio_splitter.py \
        --input_dir "$PREPROCESSED_AUDIO_DIR" \
    ...
    PROCESSING_INPUT_DIR="$PREPROCESSED_AUDIO_DIR"
fi
...
# --- Step 6: Merge Split Transcripts (if long audio splitting was used) ---
if [ "$USE_LONG_AUDIO_SPLIT" = true ]; then
    echo "--- Step 6: Merging Split Transcripts ---"
...
# --- Step 7: Evaluation ---
echo "--- Step 7: Evaluating ASR Results ---"
...
# --- Step 8: Model File Analysis with Error Logging ---
echo "--- Step 8: Analyzing Model File Processing with Error Logging ---"
...
echo "--- Step 9: Generating Integration Summary ---"
INTEGRATION_SUMMARY_FILE="$OUTPUT_DIR/integration_summary.txt"

{
    echo "Integrated ASR Pipeline with Audio Preprocessing Summary"
    echo "======================================================"
    echo "Date: $(date)"
    echo "Input Directory: $AUDIO_DIR"
    echo "Output Directory: $OUTPUT_DIR"
...
if [ "$PIPELINE_SUCCESS" = true ]; then
    echo "=== Integrated Pipeline Completed Successfully ==="
    echo ""
    ...
    echo "=== Integrated Pipeline Completed with Errors ==="
    echo ""
