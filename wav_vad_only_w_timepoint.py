...
def process_single_wav(wav_path, output_folder, vad_model):
    ...
    time_log_path = os.path.join(file_output_folder, f"{file_name}_time_point.txt")
    with open(time_log_path, "w") as time_log:
        for idx, (start_sample, end_sample) in enumerate(speech_timestamps):
            ...
            time_log.write(f"Segment {idx + 1}: {start_time:.3f}s - {end_time:.3f}s\n")
            print(f"Saved: {save_path}, Time: {start_time:.3f}s - {end_time:.3f}s")
